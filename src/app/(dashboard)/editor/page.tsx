...
export default function EditorPage() {
  ...
  const [history, setHistory] = useState<
    { code: string; timestamp: string; id?: string }[]
  >([]);
  ...
  const loadProjectVersions = async (id: string) => {
    try {
      ...
      if (versions && versions.length > 0) {
        ...
        console.log(
          `Loaded ${versionHistory.length} versions from project_mdx`,
        );
    ...
    try {
      ...
      if (filename) {
        ...
        updatedFiles = existingFiles.map((file) =>
          file.filename === filename ? { ...file, content: code } : file,
        );
      } else {
        ...
        if (existingFiles.length > 0) {
          ...
          updatedFiles = [{ filename: "documentation.mdx", content: code }];
        }
      }
  ...
  const saveVersion = async () => {
    ...
    if (projectId) {
      ...
      alert(
        "Changes saved locally. Note: This is not connected to any project.",
      );
    }
  };
  ...
  const revertToVersion = (index: number) => {
    if (index >= 0 && index < history.length) {
      ...
      if (index > 0) {
        alert(
          `You are now viewing an older version. Click "Save" to make this the latest version.`,
        );
      }
    }
  };
  ...
  const handleFileSelect = async (selectedFilename: string) => {
    ...
    try {
      ...
      const selectedFile = allFiles.find(
        (file) => file.filename === baseFilename,
      );
      ...
      if (selectedFile) {
        ...
        if (code !== selectedFile.content) {
          ...
          setHistory((prev) => [
            ...
            ...prev
              .filter((item) => item.code !== selectedFile.content)
              .slice(0, 9),
          ]);
        }
      } else {
  ...
  return (
    <div className="flex max-h-[calc(100svh-3.825rem)] overflow-hidden">
      <div
        className={cn(
      ...
      >
        {!sidebarCollapsed ? (
          ...
          <button
            ...
            className="flex h-full w-8 items-center justify-center border-r border-gray-200 bg-white hover:bg-gray-50 dark:border-gray-800 dark:bg-gray-900 dark:hover:bg-gray-800"
          >
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <div className="flex flex-1 overflow-hidden">
          ...
          {view !== "editor" && <PreviewPane code={code} view={view} />}
        </div>
      </div>
    </div>
  );
